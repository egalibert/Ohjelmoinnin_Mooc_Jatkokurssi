...
def pituuden_mukaan(reitit: list):
    def p_mukaan(alkio  :"Kiipeilyreitti"):
        return alkio.pituus

    return(sorted(reitit, key=p_mukaan, reverse=True))

def vaikeuden_mukaan(reitit: list):
    def v_mukaan(alkio :"Kiipeilyreitti"):
        return (alkio.grade, alkio.pituus)

    return(sorted(reitit, key=v_mukaan, reverse=True))
